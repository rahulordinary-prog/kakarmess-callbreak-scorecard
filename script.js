...
// The player-names row can wrap to a different height depending on name
// length, so the Call/Won/Score row's sticky offset is measured after
// every render instead of being a fixed pixel value.
function updateStickyHeaderOffset(){
  const header = document.getElementById('headerRow');
  const sub = document.getElementById('subHeaderRow');
  requestAnimationFrame(()=>{
    sub.querySelectorAll('th').forEach(th=>{
      th.style.top = header.offsetHeight + 'px';
    });
  });
}
