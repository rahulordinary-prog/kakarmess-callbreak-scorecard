...
// The header lives in its own table (separate from the scrollable body
// table) so it can never move vertically. Its columns are locked to the
// exact same pixel widths as the body table's columns via matching
// <colgroup> definitions, and its horizontal position is kept in sync with
// the body's horizontal scroll via JS. This avoids relying on
// `position: sticky` inside a table, which has known, inconsistent mobile
// browser support (especially combined with border-collapse).
function columnWidthsColgroupHtml(){
  let html = '<col style="width:56px">';
  for(let i=0;i<NUM_PLAYERS;i++){
    html += '<col style="width:64px"><col style="width:64px"><col style="width:64px">';
  }
  html += '<col style="width:52px"><col style="width:52px">';
  html += '<col style="width:36px">';
  return html;
}

function renderColgroups(){
  const html = columnWidthsColgroupHtml();
  const headCg = document.getElementById('headColgroup');
  const bodyCg = document.getElementById('bodyColgroup');
  if(headCg) headCg.innerHTML = html;
  if(bodyCg) bodyCg.innerHTML = html;
}
